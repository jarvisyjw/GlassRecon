...
├── intrinsics/ # JSON files containing camera intrinsics (including depth scale)
...
├── depths_vis/ # Visualization (PNG) showing which glass regions are missing
└── pointclouds/ # 3D point clouds (PLY) generated by projecting `depths_npy` using intrinsics
